 ...
 * C source code generated on : Wed Aug 29 12:02:12 2018
 *
...
void heli_q8_output0(void)             /* Sample time: [0.0s, 0.0s] */
{
  ...
  if (rtmIsMajorTimeStep(heli_q8_M)) {
    ...
    /* Gain: '<Root>/degtorad' incorporates:
     *  Constant: '<Root>/e_+'
     *  Sum: '<Root>/Sum'
     */
    heli_q8_B.degtorad = (heli_q8_B.Sum2 - heli_q8_P.e__Value) *
      heli_q8_P.degtorad_Gain;

    /* Sum: '<S2>/Sum' incorporates:
     *  Constant: '<Root>/e_~_c'
     */
    rtb_Sum_k = heli_q8_P.e__c_Value - heli_q8_B.degtorad;
  ...
  if ((heli_q8_DW.TimeStampA >= heli_q8_M->Timing.t[0]) &&
    ...
    rtb_Backgain = (heli_q8_B.degtorad - *lastU) / (heli_q8_M->Timing.t[0] -
      rtb_Backgain);
  }
   ...
   *  Gain: '<S6>/K_ed'
   */
  heli_q8_B.Sum = (rtb_Frontgain + heli_q8_B.K_ep) - heli_q8_P.K_ed_Gain *
    rtb_Backgain;
  if (rtmIsMajorTimeStep(heli_q8_M)) {
    ...
    /* Gain: '<S9>/Gain' */
    heli_q8_B.Gain = heli_q8_P.Gain_Gain_a * heli_q8_B.PitchCounttorad;

    /* Gain: '<Root>/degtorad1' incorporates:
     *  Constant: '<Root>/p_*1'
     *  Sum: '<Root>/Sum1'
     */
    heli_q8_B.degtorad1 = (heli_q8_B.Gain - heli_q8_P.p_1_Value) *
      heli_q8_P.degtorad1_Gain;

    /* Gain: '<S5>/K_ep1' incorporates:
     *  Sum: '<S5>/Sum'
     */
    heli_q8_B.K_ep1 = (heli_q8_B.Joystick_gain_x - heli_q8_B.degtorad1) *
      heli_q8_P.k_pp;
  }

  /* Derivative: '<Root>/Derivative1' */
  if ((heli_q8_DW.TimeStampA_d >= heli_q8_M->Timing.t[0]) &&
      (heli_q8_DW.TimeStampB_j >= heli_q8_M->Timing.t[0])) {
    rtb_Backgain = 0.0;
  } else {
    rtb_Backgain = heli_q8_DW.TimeStampA_d;
    lastU = &heli_q8_DW.LastUAtTimeA_i;
    if (heli_q8_DW.TimeStampA_d < heli_q8_DW.TimeStampB_j) {
      if (heli_q8_DW.TimeStampB_j < heli_q8_M->Timing.t[0]) {
        rtb_Backgain = heli_q8_DW.TimeStampB_j;
        lastU = &heli_q8_DW.LastUAtTimeB_i;
      }
    } else {
      if (heli_q8_DW.TimeStampA_d >= heli_q8_M->Timing.t[0]) {
        rtb_Backgain = heli_q8_DW.TimeStampB_j;
    ...
    rtb_Backgain = (heli_q8_B.degtorad1 - *lastU) / (heli_q8_M->Timing.t[0] -
      rtb_Backgain);
  }
   ...
   *  Gain: '<S5>/K_ep2'
   */
  rtb_Backgain = heli_q8_B.K_ep1 - heli_q8_P.k_pd * rtb_Backgain;

  /* Gain: '<S1>/Front gain' incorporates:
   *  Sum: '<S1>/Add'
   */
  rtb_Frontgain = (rtb_Backgain + heli_q8_B.Sum) * heli_q8_P.Frontgain_Gain;
  if (rtmIsMajorTimeStep(heli_q8_M)) {
    /* Gain: '<S6>/K_ei' */
  ...
  if (rtmIsMajorTimeStep(heli_q8_M)) {
     ...
     *  Gain: '<S11>/Gain'
     */
    heli_q8_B.Sum_a = heli_q8_P.Gain_Gain_ar * heli_q8_B.TravelCounttorad +
   ...
   *  Sum: '<S1>/Subtract'
   */
  rtb_Backgain = (heli_q8_B.Sum - rtb_Backgain) * heli_q8_P.Backgain_Gain;
...
void heli_q8_update0(void)             /* Sample time: [0.0s, 0.0s] */
{
  ...
  *lastU = heli_q8_B.degtorad;

  /* End of Update for Derivative: '<Root>/Derivative' */

  /* Update for Derivative: '<Root>/Derivative1' */
  if (heli_q8_DW.TimeStampA_d == (rtInf)) {
    heli_q8_DW.TimeStampA_d = heli_q8_M->Timing.t[0];
    lastU = &heli_q8_DW.LastUAtTimeA_i;
  } else if (heli_q8_DW.TimeStampB_j == (rtInf)) {
    heli_q8_DW.TimeStampB_j = heli_q8_M->Timing.t[0];
    lastU = &heli_q8_DW.LastUAtTimeB_i;
  } else if (heli_q8_DW.TimeStampA_d < heli_q8_DW.TimeStampB_j) {
    heli_q8_DW.TimeStampA_d = heli_q8_M->Timing.t[0];
    lastU = &heli_q8_DW.LastUAtTimeA_i;
  } else {
    heli_q8_DW.TimeStampB_j = heli_q8_M->Timing.t[0];
...
void heli_q8_derivatives(void)
{
  ...
  _rtXdot->TransferFcn_CSTATE[0U] += heli_q8_P.TransferFcn_A[0] *
    heli_q8_X.TransferFcn_CSTATE[0];
  _rtXdot->TransferFcn_CSTATE[0U] += heli_q8_P.TransferFcn_A[1] *
    heli_q8_X.TransferFcn_CSTATE[1];
  _rtXdot->TransferFcn_CSTATE[1] += heli_q8_X.TransferFcn_CSTATE[0];
  _rtXdot->TransferFcn_CSTATE[0U] += 0.0;
}
...
void heli_q8_initialize(void)
{
  ...
  heli_q8_DW.TimeStampA_d = (rtInf);
  heli_q8_DW.TimeStampB_j = (rtInf);
  ...
  heli_q8_X.TransferFcn_CSTATE[0] = 0.0;
  heli_q8_X.TransferFcn_CSTATE[1] = 0.0;
}
...
RT_MODEL_heli_q8_T *heli_q8(void)
{
  ...
  /* External mode info */
  heli_q8_M->Sizes.checksums[0] = (2421115454U);
  heli_q8_M->Sizes.checksums[1] = (712499835U);
  heli_q8_M->Sizes.checksums[2] = (3262784882U);
  heli_q8_M->Sizes.checksums[3] = (3122991932U);
  ...
  heli_q8_DW.TimeStampA_d = 0.0;
  heli_q8_DW.LastUAtTimeA_i = 0.0;
  heli_q8_DW.TimeStampB_j = 0.0;
  heli_q8_DW.LastUAtTimeB_i = 0.0;
