 ...
 * C source code generated on : Wed Oct 10 17:13:40 2018
 *
...
void heli_q8_output0(void)             /* Sample time: [0.0s, 0.0s] */
{
   ...
   */
  for (i = 0; i < 6; i++) {
    heli_q8_B.radtodeg1[i] = heli_q8_P.radtodeg1_Gain *
      heli_q8_X.Integrator_CSTATE[i];
  }
  ...
  if (rtmIsMajorTimeStep(heli_q8_M)) {
     ...
     *  Gain: '<S8>/Gain'
     */
    heli_q8_B.Sum2 = heli_q8_P.Gain_Gain * heli_q8_B.ElevationCounttorad +
   ...
   */
  heli_q8_B.Gain = (heli_q8_P.ElevationTransferFcn_C *
  ...
  if (rtmIsMajorTimeStep(heli_q8_M)) {
     ...
     *  Gain: '<S12>/Gain'
     */
    heli_q8_B.Sum = heli_q8_P.Gain_Gain_ar * heli_q8_B.TravelCounttorad +
  ...
  if (rtmIsMajorTimeStep(heli_q8_M)) {
    ...
    for (i = 0; i < 6; i++) {
      rtb_TmpSignalConversionAtToFile[i] = heli_q8_B.radtodeg1[i];
    }

    rtb_TmpSignalConversionAtToFile[6] = heli_q8_B.Gain_i;
    rtb_TmpSignalConversionAtToFile[7] = heli_q8_B.Sum1;
    rtb_TmpSignalConversionAtToFile[8] = heli_q8_B.Sum2;
    rtb_TmpSignalConversionAtToFile[9] = heli_q8_B.Gain;
    rtb_TmpSignalConversionAtToFile[10] = heli_q8_B.Sum;
    rtb_TmpSignalConversionAtToFile[11] = heli_q8_B.Gain_d;
    ...
    if (rtmIsMajorTimeStep(heli_q8_M)) {
      {
        if (!(++heli_q8_DW.ToFile_IWORK.Decimation % 1) &&
            (heli_q8_DW.ToFile_IWORK.Count*13)+1 < 100000000 ) {
          ...
          if (fp != (NULL)) {
            ...
            if (fwrite(u, sizeof(real_T), 13, fp) != 13) {
              rtmSetErrorStatus(heli_q8_M,
                                "Error writing to MAT-file estimation_vs_measured_integral_feed_to_lqr2_joystick_final.mat");
              return;
            }

            if (((++heli_q8_DW.ToFile_IWORK.Count)*13)+1 >= 100000000) {
              (void)fprintf(stdout,
                            "*** The ToFile block will stop logging data before\n"
                            "    the simulation has ended, because it has reached\n"
                            "    the maximum number of elements (100000000)\n"
                            "    allowed in MAT-file estimation_vs_measured_integral_feed_to_lqr2_joystick_final.mat.\n");
            }
          }
        }
      }
    ...
    heli_q8_B.Constant = heli_q8_P.v_s_star;
   ...
   *  Integrator: '<S6>/Integrator'
   */
  tmp[0] = heli_q8_X.Integrator_CSTATE[0];
  tmp[1] = heli_q8_X.Integrator_CSTATE[1];
  tmp[2] = heli_q8_X.Integrator_CSTATE[3];
  tmp[3] = heli_q8_X.Integrator1_CSTATE;
  tmp[4] = heli_q8_X.Integrator_CSTATE_c;
  ...
  for (i = 0; i < 2; i++) {
    ...
    rtb_u[i] = heli_q8_B.Referencefeedforward[i] - rtb_Frontgain;
  }

  /* End of Sum: '<S4>/Sum3' */

  /* Sum: '<Root>/Sum' */
  rtb_Frontgain = rtb_u[0] + heli_q8_B.Constant;
  if (rtmIsMajorTimeStep(heli_q8_M)) {
  }
   ...
   *  Sum: '<S1>/Add'
   */
  rtb_L_idx_2 = (rtb_u[1] + rtb_Frontgain) * heli_q8_P.Frontgain_Gain;

  /* Saturate: '<S2>/Front motor: Saturation' */
  if (rtb_L_idx_2 > heli_q8_P.FrontmotorSaturation_UpperSat) {
    heli_q8_B.FrontmotorSaturation = heli_q8_P.FrontmotorSaturation_UpperSat;
  } else if (rtb_L_idx_2 < heli_q8_P.FrontmotorSaturation_LowerSat) {
    heli_q8_B.FrontmotorSaturation = heli_q8_P.FrontmotorSaturation_LowerSat;
  } else {
    heli_q8_B.FrontmotorSaturation = rtb_L_idx_2;
  }
   ...
   *  Sum: '<S1>/Subtract'
   */
  rtb_L_idx_2 = (rtb_Frontgain - rtb_u[1]) * heli_q8_P.Backgain_Gain;

  /* Saturate: '<S2>/Back motor: Saturation' */
  if (rtb_L_idx_2 > heli_q8_P.BackmotorSaturation_UpperSat) {
    heli_q8_B.BackmotorSaturation = heli_q8_P.BackmotorSaturation_UpperSat;
  } else if (rtb_L_idx_2 < heli_q8_P.BackmotorSaturation_LowerSat) {
    heli_q8_B.BackmotorSaturation = heli_q8_P.BackmotorSaturation_LowerSat;
  } else {
    heli_q8_B.BackmotorSaturation = rtb_L_idx_2;
  }
   ...
   *  Integrator: '<S6>/Integrator'
   */
  heli_q8_B.Sum1_n = heli_q8_X.Integrator_CSTATE[0] - heli_q8_B.Joystick_gain_x;

  /* Sum: '<S4>/Sum2' incorporates:
   *  Integrator: '<S6>/Integrator'
   */
  heli_q8_B.Sum2_f = heli_q8_X.Integrator_CSTATE[3] - heli_q8_B.Joystick_gain_y;

  /* Gain: '<Root>/degtorad3' */
  rtb_Frontgain = heli_q8_P.degtorad3_Gain * heli_q8_B.Gain_i;
  rtb_L_idx_2 = heli_q8_P.degtorad3_Gain * heli_q8_B.Sum2;
  rtb_L_idx_4 = heli_q8_P.degtorad3_Gain * heli_q8_B.Sum;
  for (i = 0; i < 6; i++) {
    /* Gain: '<S6>/B' incorporates:
     *  Sum: '<S6>/Sum'
     */
    tmp_0[i] = heli_q8_P.B_e[i + 6] * rtb_u[1] + heli_q8_P.B_e[i] * rtb_u[0];

    /* Gain: '<S6>/L' incorporates:
     *  SignalConversion: '<S6>/TmpSignal ConversionAtLInport1'
     *  Sum: '<S6>/Sum'
     */
    tmp_1[i] = heli_q8_P.L[i + 12] * rtb_L_idx_4 + (heli_q8_P.L[i + 6] *
      rtb_L_idx_2 + heli_q8_P.L[i] * rtb_Frontgain);
  }
  ...
  for (i = 0; i < 6; i++) {
    rtb_Frontgain = 0.0;
    for (i_0 = 0; i_0 < 6; i_0++) {
      rtb_Frontgain += heli_q8_P.A_est[6 * i_0 + i] *
        heli_q8_X.Integrator_CSTATE[i_0];
    }

    heli_q8_B.Sum_c[i] = (tmp_0[i] + tmp_1[i]) + rtb_Frontgain;
  }
}
...
void heli_q8_derivatives(void)
{
  ...
  for (i = 0; i < 6; i++) {
    _rtXdot->Integrator_CSTATE[i] = heli_q8_B.Sum_c[i];
  }
  ...
  _rtXdot->Integrator1_CSTATE = heli_q8_B.Sum1_n;

  /* Derivatives for Integrator: '<S4>/Integrator' */
  _rtXdot->Integrator_CSTATE_c = heli_q8_B.Sum2_f;
}
...
void heli_q8_initialize(void)
{
  ...
  {
    char fileName[509] =
      "estimation_vs_measured_integral_feed_to_lqr2_joystick_final.mat";
    FILE *fp = (NULL);
    if ((fp = fopen(fileName, "wb")) == (NULL)) {
      rtmSetErrorStatus(heli_q8_M,
                        "Error creating .mat file estimation_vs_measured_integral_feed_to_lqr2_joystick_final.mat");
      return;
    }

    if (rt_WriteMat4FileHeader(fp,13,0,"estimationvsmeasured")) {
      rtmSetErrorStatus(heli_q8_M,
                        "Error writing mat file header to file estimation_vs_measured_integral_feed_to_lqr2_joystick_final.mat");
      return;
    }
...

  {
    ...
    heli_q8_X.Integrator_CSTATE_c = heli_q8_P.Integrator_IC_k;
  }
}
...
void heli_q8_terminate(void)
{
  ...
  /* Terminate for ToFile: '<S5>/To File' */
  {
    ...
    if (fp != (NULL)) {
      ...
      if (fclose(fp) == EOF) {
        rtmSetErrorStatus(heli_q8_M,
                          "Error closing MAT-file estimation_vs_measured_integral_feed_to_lqr2_joystick_final.mat");
        return;
      }

      if ((fp = fopen(fileName, "r+b")) == (NULL)) {
        rtmSetErrorStatus(heli_q8_M,
                          "Error reopening MAT-file estimation_vs_measured_integral_feed_to_lqr2_joystick_final.mat");
        return;
      }

      if (rt_WriteMat4FileHeader(fp, 13, heli_q8_DW.ToFile_IWORK.Count,
           "estimationvsmeasured")) {
        rtmSetErrorStatus(heli_q8_M,
                          "Error writing header for estimationvsmeasured to MAT-file estimation_vs_measured_integral_feed_to_lqr2_joystick_final.mat");
      }
...
RT_MODEL_heli_q8_T *heli_q8(void)
{
  ...
  /* External mode info */
  heli_q8_M->Sizes.checksums[0] = (2602565396U);
  heli_q8_M->Sizes.checksums[1] = (344285415U);
  heli_q8_M->Sizes.checksums[2] = (292242389U);
  heli_q8_M->Sizes.checksums[3] = (1745968370U);
...

  {
    int_T i;
    for (i = 0; i < 6; i++) {
      heli_q8_B.radtodeg1[i] = 0.0;
    }

    for (i = 0; i < 6; i++) {
      heli_q8_B.Sum_c[i] = 0.0;
    }
  ...
  heli_q8_M->Sizes.numBlockPrms = (230);/* Sum of parameter "widths" */
  return heli_q8_M;
}
