 ...
 * C source code generated on : Wed Oct 10 13:05:42 2018
 *
...
void heli_q8_output0(void)             /* Sample time: [0.0s, 0.0s] */
{
          ...
          if (fp != (NULL)) {
            ...
            if (fwrite(u, sizeof(real_T), 13, fp) != 13) {
              rtmSetErrorStatus(heli_q8_M,
                                "Error writing to MAT-file estimationvsmeasuredSTEP2.mat");
              return;
            }
            ...
            if (((++heli_q8_DW.ToFile_IWORK.Count)*13)+1 >= 100000000) {
              (void)fprintf(stdout,
                            ...
                            "    allowed in MAT-file estimationvsmeasuredSTEP2.mat.\n");
            }
          }
        }
      }
   ...
   */
  for (i = 0; i < 2; i++) {
    rtb_u[i] = heli_q8_B.Referencefeedforward[i] - ((heli_q8_P.K[i + 2] *
      heli_q8_X.Integrator_CSTATE[1] + heli_q8_P.K[i] *
      heli_q8_X.Integrator_CSTATE[0]) + heli_q8_P.K[i + 4] *
      heli_q8_X.Integrator_CSTATE[3]);
  }
   ...
   *  Sum: '<S1>/Add'
   */
  rtb_L_idx_0 = (rtb_u[1] + rtb_u[0]) * heli_q8_P.Frontgain_Gain;

  /* Saturate: '<S2>/Front motor: Saturation' */
  if (rtb_L_idx_0 > heli_q8_P.FrontmotorSaturation_UpperSat) {
    heli_q8_B.FrontmotorSaturation = heli_q8_P.FrontmotorSaturation_UpperSat;
  } else if (rtb_L_idx_0 < heli_q8_P.FrontmotorSaturation_LowerSat) {
    heli_q8_B.FrontmotorSaturation = heli_q8_P.FrontmotorSaturation_LowerSat;
  } else {
    heli_q8_B.FrontmotorSaturation = rtb_L_idx_0;
  }
   ...
   *  Sum: '<S1>/Subtract'
   */
  rtb_L_idx_0 = (rtb_u[0] - rtb_u[1]) * heli_q8_P.Backgain_Gain;

  /* Saturate: '<S2>/Back motor: Saturation' */
  if (rtb_L_idx_0 > heli_q8_P.BackmotorSaturation_UpperSat) {
    heli_q8_B.BackmotorSaturation = heli_q8_P.BackmotorSaturation_UpperSat;
  } else if (rtb_L_idx_0 < heli_q8_P.BackmotorSaturation_LowerSat) {
    heli_q8_B.BackmotorSaturation = heli_q8_P.BackmotorSaturation_LowerSat;
  } else {
    heli_q8_B.BackmotorSaturation = rtb_L_idx_0;
  }
  ...
  for (i = 0; i < 6; i++) {
     ...
     *  Sum: '<S6>/Sum'
     */
    tmp[i] = heli_q8_P.B_e[i + 6] * rtb_u[1] + heli_q8_P.B_e[i] * rtb_u[0];

    /* Gain: '<S6>/L' incorporates:
     *  SignalConversion: '<S6>/TmpSignal ConversionAtLInport1'
     *  Sum: '<S6>/Sum'
     */
    tmp_0[i] = heli_q8_P.L[i + 12] * rtb_L_idx_4 + (heli_q8_P.L[i + 6] *
      rtb_L_idx_2 + heli_q8_P.L[i] * rtb_L_idx_0);
  }
  ...
  for (i = 0; i < 6; i++) {
    rtb_L_idx_0 = 0.0;
    for (i_0 = 0; i_0 < 6; i_0++) {
      rtb_L_idx_0 += heli_q8_P.A_est[6 * i_0 + i] *
        heli_q8_X.Integrator_CSTATE[i_0];
    }

    heli_q8_B.Sum_c[i] = (tmp[i] + tmp_0[i]) + rtb_L_idx_0;
  }
}
...
void heli_q8_initialize(void)
{
  ...
  {
    char fileName[509] = "estimationvsmeasuredSTEP2.mat";
    FILE *fp = (NULL);
    if ((fp = fopen(fileName, "wb")) == (NULL)) {
      rtmSetErrorStatus(heli_q8_M,
                        "Error creating .mat file estimationvsmeasuredSTEP2.mat");
      return;
    }

    if (rt_WriteMat4FileHeader(fp,13,0,"estimationvsmeasured")) {
      rtmSetErrorStatus(heli_q8_M,
                        "Error writing mat file header to file estimationvsmeasuredSTEP2.mat");
      return;
    }
...
void heli_q8_terminate(void)
{
  ...
  /* Terminate for ToFile: '<S5>/To File' */
  {
    ...
    if (fp != (NULL)) {
      ...
      if (fclose(fp) == EOF) {
        rtmSetErrorStatus(heli_q8_M,
                          "Error closing MAT-file estimationvsmeasuredSTEP2.mat");
        return;
      }

      if ((fp = fopen(fileName, "r+b")) == (NULL)) {
        rtmSetErrorStatus(heli_q8_M,
                          "Error reopening MAT-file estimationvsmeasuredSTEP2.mat");
        return;
      }
      ...
      if (rt_WriteMat4FileHeader(fp, 13, heli_q8_DW.ToFile_IWORK.Count,
        ...
        rtmSetErrorStatus(heli_q8_M,
                          "Error writing header for estimationvsmeasured to MAT-file estimationvsmeasuredSTEP2.mat");
      }
...
RT_MODEL_heli_q8_T *heli_q8(void)
{
  ...
  /* External mode info */
  heli_q8_M->Sizes.checksums[0] = (853330034U);
  heli_q8_M->Sizes.checksums[1] = (920940058U);
  heli_q8_M->Sizes.checksums[2] = (4246722026U);
  heli_q8_M->Sizes.checksums[3] = (527756327U);
  ...
  heli_q8_M->Sizes.numBlockPrms = (225);/* Sum of parameter "widths" */
  return heli_q8_M;
}
